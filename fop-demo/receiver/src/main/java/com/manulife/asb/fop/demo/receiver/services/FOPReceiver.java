...
public class FOPReceiver implements CommandLineRunner{
    ...
    public void run(String... args) {
        try{
            ...
            int[] processSeconds = {1, 3, 5};

            LOGGER.info("[Start::DemoReceiver::run()::Step-2]");
            for(int i=0; i<ctxs.length; i = (i == ctxs.length - 1) ? 0 : i+1){
                processSessionMsg(sessionClient, ctxs[i], waitSeconds[i], processSeconds[i]);
            }
        }catch (Exception e){
    ...
    private void processSessionMsg(ServiceBusSessionReceiverClient sessionClient, String ctx, int waitSeconds, int processSeconds){
        ...
        try {
            for(ServiceBusReceivedMessage message : client.receiveMessages(batchsize, Duration.ofSeconds(waitSeconds))){
                try {
                    ...
                    Thread.sleep(processSeconds * 100);

                    //Save to DB to view the effect
                    repo.save(new FOPMsg(0, message.getMessageId(), message.getSessionId(), msg));

                    client.complete(message); //ack the message
                } catch (Exception e) {
